...
def postprocess(df, model, resultdir=None, annual=False):
  ...
  :param model: (REVIN object) model object, post-optimization
  ...
  demand_nodes = pd.read_csv('data/demand_nodes_region.csv', index_col = 0)
...
def aggregate_regions(fp):
  ...
  demand_nodes = pd.read_csv('data/demand_nodes_region.csv', index_col = 0)
  portfolio = pd.read_csv('data/portfolio_region.csv', index_col = 0)
