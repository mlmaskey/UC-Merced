...
class REVIN():
  ...
  def __init__(self, linksfile, ic=None, log_name="revin"):
    """
    This program is a variant of CALVIN specifically to cookie-cut version of California Water Network
    Initialize REVIN model object.
    ...
    :param log_name: A name for a logger - will be used to keep logs from different model runs separate in files.
                Defaults to "revin", which results in a log file in the current working directory named "revin.log".
                You can change this each time you instantiate the REVIN class if you want to output separate logs
                for different runs. Otherwise, all results will be appended to the log file (not overwritten). If you
                run multiple copies of REVIN simultaneously, make sure to change this, or you could get errors writing
                to the log file.
    ...
    if not self.log.hasHandlers():  # hasHandlers will only be True if someone already called REVIN with the same log_name in the same session
