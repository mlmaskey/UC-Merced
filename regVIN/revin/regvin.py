...
class REGVIN():
  ...
  def __init__(self, linksfile , inboundNode, outboundNode, ic=None, log_name="regvin"):
    """
    This program is a variant of CALVIN specifically to a specific of California Water Network
    Initialize REGVIN model object.
    ...
    :param log_name: A name for a logger - will be used to keep logs from different model runs separate in files.
                Defaults to "regvin", which results in a log file in the current working directory named "regvin.log".
                You can change this each time you instantiate the REGVIN class if you want to output separate logs
                for different runs. Otherwise, all results will be appended to the log file (not overwritten). If you
                run multiple copies of REGVIN simultaneously, make sure to change this, or you could get errors writing
                to the log file.
    ...
    :returns: REGVIN model object
    """
    def fixInbound(df):
        """
        Modifies the inbound link with higher penalties to fix the infeasibility

        Mahesh L Maskey August 29,2020
        Parameters
        ----------
        df : dataframe from original link file
        ...
        """
        inboundLinks = df[df.i.str.contains('INBOUND')]
        uniqueNode =inboundLinks.i.unique()
        iAllinboundLinkDf = pd.DataFrame()
        for l in uniqueNode:
            inboundLinks_Filter = inboundLinks[inboundLinks.i==l]
            # inboundLinks_Filter = inboundLinks[inboundLinks.i==uniqueNode[1]]
            uniqueNodej =inboundLinks_Filter.j.unique()
            iAllLinkDf = pd.DataFrame()
            for m  in uniqueNodej:
                inboundLinks_Filterj = inboundLinks_Filter[inboundLinks_Filter.j==m]
                # inboundLinks_Filterj = inboundLinks_Filter[inboundLinks_Filter.j==uniqueNodej[1]]
                iLinkDf = pd.DataFrame(0, index=np.arange(3), columns=inboundLinks_Filterj.columns)
                iLinkDf.i = inboundLinks_Filterj.iloc[0,0]
                iLinkDf.j = inboundLinks_Filterj.iloc[0,1]
                iLinkDf.k = np.arange(3)
                iLinkDf.cost = [-1e6, 0, 1e6]
                iLinkDf.amplitude = 1
                lb_ub = np.sum(inboundLinks_Filterj.upper_bound)
                iLinkDf.lower_bound = [0, lb_ub, lb_ub]
                iLinkDf.upper_bound = [lb_ub, lb_ub, 1e12]
                iAllLinkDf = pd.concat([iAllLinkDf, iLinkDf], axis = 0)
            iAllinboundLinkDf = pd.concat([iAllinboundLinkDf, iAllLinkDf], axis = 0)
        
        dropLinks = df.drop(inboundLinks.index)
        newLinks = pd.concat([iAllinboundLinkDf, dropLinks], axis = 0)
        del df
        df = newLinks
        return (df)

    def fixOutbound(df):
        outboundLinks = df[df.j.str.contains('OUTBOUND')]
        uniqueNode =outboundLinks.j.unique()
        jAlloutboundLinkDf = pd.DataFrame()
        for l in uniqueNode:
            outboundLinks_Filter = outboundLinks[outboundLinks.j==l]
            uniqueNodei =outboundLinks_Filter.i.unique()
            jAllLinkDf = pd.DataFrame()
            for m  in uniqueNodei:
                outboundLinks_Filteri = outboundLinks_Filter[outboundLinks_Filter.i==m]
                jLinkDf = pd.DataFrame(0, index=np.arange(3), columns=outboundLinks_Filteri.columns)
                jLinkDf.i = outboundLinks_Filteri.iloc[0,0]
                jLinkDf.j = outboundLinks_Filteri.iloc[0,1]
                jLinkDf.k = np.arange(3)
                jLinkDf.cost = [-1e6, 0, 1e6]
                jLinkDf.amplitude = 1
                lb_ub = np.sum(outboundLinks_Filteri.upper_bound)
                jLinkDf.lower_bound = [0, lb_ub, lb_ub]
                jLinkDf.upper_bound = [lb_ub, lb_ub, 1e12]
                jAllLinkDf = pd.concat([jAllLinkDf, jLinkDf], axis = 0)
            jAlloutboundLinkDf = pd.concat([jAlloutboundLinkDf, jAllLinkDf], axis = 0)
            
        dropLinks = df.drop(outboundLinks.index)
        newLinks = pd.concat([jAlloutboundLinkDf, dropLinks], axis = 0)
        del df
        df = newLinks
        return (df)

    def addBounds(df, iNboundLink, outboundLink):
        
        
        inBoundLinks = pd.DataFrame(columns=df.columns)        
        for link in iNboundLink:
            iLinkDf = pd.DataFrame(0, index=np.arange(12), columns=df.columns)
            inboundLinks = df[df.i.str.contains(link)]
            jNode = inboundLinks.i.unique()
            jNode = [i for i in jNode if i.startswith(link)]
            inode = []
            for node in jNode:
                iNode = node.replace(link, 'INBOUND')
                inode.append(iNode)    
            iNode = np.array(inode)
            iLinkDf.i = iNode
            iLinkDf.j = jNode
            iLinkDf.k = 0
            iLinkDf.cost = 0
            iLinkDf.amplitude = 1
            iLinkDf.lower_bound = 0
            iLinkDf.upper_bound = 0
            inBoundLinks = pd.concat([inBoundLinks, iLinkDf], axis = 0)
        
        inboundDf = fixInbound(inBoundLinks)
                    
            
            
        outBoundLinkDf = pd.DataFrame(columns=df.columns)        
        for link in outboundLink:
            jLinkDf = pd.DataFrame(0, index=np.arange(12), columns=df.columns)
            outboundLinks = df[df.i.str.contains(link)]
            iNode = outboundLinks.i.unique()
            iNode = [i for i in iNode if i.startswith(link)]
            jnode = []
            for node in iNode:
                jNode = node.replace(link, 'OUTBOUND')
                jnode.append(jNode)    
            jNode = np.array(jnode)
            jLinkDf.i = iNode
            jLinkDf.j = jNode
            jLinkDf.k = 0
            jLinkDf.cost = 0
            jLinkDf.amplitude = 1
            jLinkDf.lower_bound = 0
            jLinkDf.upper_bound = 0
            outBoundLinkDf = pd.concat([outBoundLinkDf, jLinkDf], axis = 0)
        
        outBoundDf = fixOutbound(outBoundLinkDf)            
        newDF = pd.concat([inboundDf, df, outBoundDf], axis = 0)
    ...
    def sourceSinck(df):
        inboundLinks = df[df.i.str.contains('INBOUND')]
        inboundNode = inboundLinks.i.unique()
        sourceLinkDf = pd.DataFrame()
        for inNode in inboundNode:
            sourceLink = pd.DataFrame(0, index=np.arange(1), columns=df.columns)
            sourceLink.i = 'SOURCE'
            sourceLink.j = inNode
            sourceLink.k = 0
            sourceLink.amplitude = 1
            sourceLink.cost = 0
            sourceLink.lower_bound = 0
            sourceLink.upper_bound = 1e12
            sourceLinkDf = pd.concat([sourceLinkDf, sourceLink], axis = 0)
            
        outboundLinks = df[df.j.str.contains('OUTBOUND')]
        outboundNode = outboundLinks.j.unique()
        sinkLinkDf = pd.DataFrame()
        for outdNode in outboundNode:
            sinkLink = pd.DataFrame(0, index=np.arange(1),  columns=df.columns)
            sinkLink.i = outdNode
            sinkLink.j = 'SINK'
            sinkLink.k = 0
            sinkLink.amplitude = 1
            sinkLink.cost = 0
            sinkLink.lower_bound = 0
            sinkLink.upper_bound = 1e12
            sinkLinkDf = pd.concat([sinkLinkDf, sinkLink], axis = 0)
        df = pd.concat([sourceLinkDf, df,  sinkLinkDf], axis=0)
        df.index = np.arange(df.shape[0])
        return (df)

    # set up logging code
    self.log = logging.getLogger(log_name)
    if not self.log.hasHandlers():  # hasHandlers will only be True if someone already called REGVIN with the same log_name in the same session
    ...
    df =  addBounds(df, inboundNode, outboundNode)
    df = sourceSinck(df)
    ...
    df.to_csv(self.linksfile+'-new.csv')
    # self.T = len(self.df)
    SR_stats = pd.read_csv('data/SR_stats.csv', index_col=0).to_dict()
