...
def postprocess(df, model, resultdir=None, annual=False):
  ...
  df_model = model_to_dataframe(model)
  ...
  df_model.to_csv(resultdir + '/MatrixModel.csv')
...
def aggregate_regions(fp):
  ...
  sc.to_csv(fp + '/shortage_cost_original.csv')
  sv.to_csv(fp + '/shortage_volume_original.csv')
  flow.to_csv(fp + '/flow_original.csv')
